...
namespace
{
std::string mdcell_force_text(const MdCell& mdcell)
{
    ...
    for (const LocalAtom& atom : mdcell.owned_atoms())
    {
        ...
        output << " " << mdcell.type_labels()[static_cast<std::size_t>(atom.type)]
               << atom.type_index + 1 << " " << fx << " " << fy << " " << fz << "\n";
    }
    return output.str();
}
...
void print_mdcell_force(const MdCell& mdcell)
{
    ...
    const std::string local_text = mdcell_force_text(mdcell);
    if (rank == 0)
    {
...
void ESolver_LJ::cal_stress(BaseCell& cell, ModuleBase::matrix& stress)
{
    ...
    const double external_stress[3] = {PARAM.inp.press1, PARAM.inp.press2, PARAM.inp.press3};
    for (int i = 0; i < 3; ++i)
    {
