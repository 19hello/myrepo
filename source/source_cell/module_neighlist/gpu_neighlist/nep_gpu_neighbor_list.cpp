#include "nep_gpu_neighbor_list.h"

#include "source_base/timer.h"

#include <cstddef>

int nep_gpu_build_neighbor_list(int nall,
                                double cutoff,
...
int nep_gpu_filter_neighbor_list(int nall,
                                 double cutoff,
                                 const double* position,
...
int nep_gpu_build_neighbor_list_device(int nall,
                                       double cutoff,
                                       const double* position,
...
int nep_gpu_filter_neighbor_list_device(int nall,
                                        double cutoff,
                                        const double* position,
                                        int candidate_max_neighbors,
...
int nep_gpu_upload_neighbor_list(int nall,
                                 int max_neighbors,
...
void nep_gpu_release_neighbor_list(void* device_neighbor_count,
                                   void* device_neighbor_indices);

namespace ModuleESolver
{

NEP_GPU_NeighborList::~NEP_GPU_NeighborList()
{
    ...
    {
        nep_gpu_release_neighbor_list(device_neighbor_count_, device_neighbor_indices_);
    }
    nep_gpu_release_neighbor_list(candidate_device_count_, candidate_device_indices_);
    nep_gpu_release_neighbor_list(filtered_device_count_, filtered_device_indices_);
}

bool NEP_GPU_NeighborList::build_device(int nall,
                                        double cutoff,
                                        const std::vector<double>& position,
                                        int& max_neighbors,
                                        std::string& error)
{
    ModuleBase::timer::start("NEP_GPU_NeighborList", "build_device");
    if (nall <= 0 || position.size() != static_cast<std::size_t>(3 * nall))
    {
        error = "invalid GPU device neighbor-list input";
        ModuleBase::timer::end("NEP_GPU_NeighborList", "build_device");
        return false;
    }
    ...
    const int status = nep_gpu_build_neighbor_list_device(nall,
                                                          cutoff,
                                                          position.data(),
                                                          &new_count,
                                                          &new_indices,
...
bool NEP_GPU_NeighborList::filter_device(int nall,
                                         double cutoff,
                                         const std::vector<double>& position,
                                         int candidate_max_neighbors,
                                         int& max_neighbors,
                                         std::string& error)
{
    ModuleBase::timer::start("NEP_GPU_NeighborList", "filter_device");
    if (nall <= 0 || position.size() != static_cast<std::size_t>(3 * nall)
        || candidate_device_nall_ != nall
        ...
        || (candidate_max_neighbors > 0 && candidate_device_indices_ == nullptr))
    {
        ...
        ModuleBase::timer::end("NEP_GPU_NeighborList", "filter_device");
        return false;
    }
    ...
    void* new_count = nullptr;
    void* new_indices = nullptr;
    const int status = nep_gpu_filter_neighbor_list_device(
        nall,
        cutoff,
        position.data(),
...
void NEP_GPU_NeighborList::use_candidate_device()
{
    nep_gpu_release_neighbor_list(filtered_device_count_, filtered_device_indices_);
    filtered_device_count_ = nullptr;
    filtered_device_indices_ = nullptr;
...
bool NEP_GPU_NeighborList::upload(int nall,
                                  int max_neighbors,
                                  const std::vector<int>& neighbor_count,
                                  const std::vector<int>& neighbor_indices,
                                  std::string& error)
{
    ModuleBase::timer::start("NEP_GPU_NeighborList", "upload");
                                             ...
                                             * static_cast<std::size_t>(max_neighbors))
    {
        ...
        ModuleBase::timer::end("NEP_GPU_NeighborList", "upload");
        return false;
    }
    ...
    const int status = nep_gpu_upload_neighbor_list(nall,
                                                    max_neighbors,
                                                    neighbor_count.data(),
...
const void* NEP_GPU_NeighborList::device_neighbor_count() const
{
    return device_neighbor_count_;
}

const void* NEP_GPU_NeighborList::device_neighbor_indices() const
{
    return device_neighbor_indices_;
}

int NEP_GPU_NeighborList::device_nall() const
{
    return device_nall_;
}

int NEP_GPU_NeighborList::device_max_neighbors() const
{
    return device_max_neighbors_;
}

bool NEP_GPU_NeighborList::build(int nall,
                                 double cutoff,
                                 const std::vector<double>& position,
                                 ...
                                 std::string& error) const
{
    ...
    max_neighbors = 0;
    char message[512] = {0};
    const int status = nep_gpu_build_neighbor_list(nall,
                                                   cutoff,
                                                   position.data(),
                                                   neighbor_count.data(),
                                                   NULL,
    ...
    if (max_neighbors > 0)
    {
        const int fill_status = nep_gpu_build_neighbor_list(nall,
                                                            cutoff,
                                                            position.data(),
                                                            neighbor_count.data(),
...
bool NEP_GPU_NeighborList::filter(int nall,
                                  double cutoff,
                                  const std::vector<double>& position,
                                  ...
                                  std::string& error) const
{
    ModuleBase::timer::start("NEP_GPU_NeighborList", "filter");
    if (nall <= 0 || cutoff <= 0.0
        || position.size() != static_cast<std::size_t>(3 * nall)
                                         ...
                                         * static_cast<std::size_t>(candidate_max_neighbors))
    {
        ...
        ModuleBase::timer::end("NEP_GPU_NeighborList", "filter");
        return false;
    }
    ...
    max_neighbors = 0;
    char message[512] = {0};
    const int status = nep_gpu_filter_neighbor_list(nall,
                                                    cutoff,
                                                    position.data(),
