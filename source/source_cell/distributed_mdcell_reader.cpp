...
void broadcast_stru_metadata(MdStruMetadata& metadata, MPI_Comm comm)
{
    int ntype = static_cast<int>(metadata.species.size());
    MPI_Bcast(&ntype, 1, MPI_INT, 0, comm);
    metadata.species.resize(static_cast<std::size_t>(ntype));
    for (int it = 0; it < ntype; ++it)
    {
        MdStruSpecies& species = metadata.species[static_cast<std::size_t>(it)];
        MPI_Bcast(&species.mass, 1, MPI_DOUBLE, 0, comm);
        MPI_Bcast(&species.start_mag, 1, MPI_DOUBLE, 0, comm);
        MPI_Bcast(&species.atom_count, 1, MPI_INT, 0, comm);
        std::string* fields[] = {&species.label, &species.pseudo_file, &species.pseudo_type, &species.orbital_file};
        for (int ifield = 0; ifield < 4; ++ifield)
        {
            int length = static_cast<int>(fields[ifield]->size());
            MPI_Bcast(&length, 1, MPI_INT, 0, comm);
            fields[ifield]->resize(static_cast<std::size_t>(length));
            if (length > 0) MPI_Bcast(&(*fields[ifield])[0], length, MPI_CHAR, 0, comm);
        }
    }
    int descriptor_length = static_cast<int>(metadata.descriptor_file.size());
    MPI_Bcast(&descriptor_length, 1, MPI_INT, 0, comm);
    metadata.descriptor_file.resize(static_cast<std::size_t>(descriptor_length));
    if (descriptor_length > 0) MPI_Bcast(&metadata.descriptor_file[0], descriptor_length, MPI_CHAR, 0, comm);
}
                                ...
                                int& nat)
{
    ...
    if (rank == 0)
    {
        ...
        for (std::size_t it = 0; it < metadata.labels.size(); ++it)
        {
            ...
            const std::string magnetism = next_data_line(ifs, "magnetism");
            std::istringstream magnetism_stream(magnetism);
            magnetism_stream >> metadata.stru_metadata.species[it].start_mag;
                                             ...
                                             double skin_bohr)
{
    ...
    distribute_atoms_from_root(ifs, metadata, comm, cutoff_bohr, skin_bohr, owned_atoms, nat);
    MPI_Bcast(&nat, 1, MPI_INT, 0, comm);
    broadcast_stru_metadata(metadata.stru_metadata, comm);
